...
import { collection, getDocs, addDoc, updateDoc, deleteDoc, doc, query, limit, orderBy } from 'firebase/firestore';
...
export function GamesTab() {
  ...
  const fetchGames = async () => {
    ...
    try {
      ...
      setGames(querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as BoardGame[]);
    } catch (error) { toast({ title: "Erreur de chargement des jeux", variant: "destructive" }); } 
    finally { setIsLoading(false); }
  };

  const seedGamesDatabase = async () => {
    setIsSeeding(true);
    toast({ title: "Lancement du remplissage...", description: "Envoi des jeux à Firestore." });
    try {
      const gamesRef = collection(db, 'games');
      const q = query(gamesRef, limit(1));
      const snapshot = await getDocs(q);
      if (!snapshot.empty) {
        toast({ title: "Opération annulée", description: "La collection 'games' contient déjà des données.", variant: "destructive" });
        return;
      }
      let count = 0;
      for (const game of mockBoardGames) {
        const { id, ...gameData } = game;
        await addDoc(gamesRef, gameData);
        count++;
      }
      toast({ title: "Remplissage terminé !", description: `${count} jeux ont été ajoutés.` });
      await fetchGames();
    } catch (error) { toast({ title: "Erreur de remplissage", variant: "destructive" }); } 
    finally { setIsSeeding(false); }
  };

  const handleSaveGame = async (gameData: GameFormValues) => {
    setIsLoading(true);
    const dataToSave = { ...gameData, publicationYear: Number(gameData.publicationYear) || undefined };
    try {
      if (isAddingGame || !gameData.id) {
        const { id, ...newGameData } = dataToSave;
        await addDoc(collection(db, "games"), newGameData);
        toast({ title: "Jeu Ajouté", description: `Le jeu "${newGameData.name}" a été créé.` });
      } else {
        const gameDocRef = doc(db, 'games', gameData.id);
        const { id, ...updatedGameData } = dataToSave;
        await updateDoc(gameDocRef, updatedGameData);
        toast({ title: "Jeu Modifié", description: `Le jeu "${gameData.name}" a été mis à jour.` });
      }
      await fetchGames();
    } catch (error) { toast({ title: "Erreur de sauvegarde", variant: "destructive" }); }
    finally { setIsLoading(false); setIsGameFormDialogOpen(false); }
  };
  ...
  const handleDeleteGame = async (gameId: string, gameName: string) => {
    ...
    try {
      ...
      toast({ title: "Jeu Supprimé", description: `Le jeu "${gameName}" a été supprimé.` });
      await fetchGames();
    } catch (error) { toast({ title: "Erreur de suppression", variant: "destructive" }); }
    finally { setIsLoading(false); }
  };
  ...
  const allCategorizedTagsForGames = useMemo(() => { const c: Record<string, Set<string>> = {}; (Object.keys(TAG_CATEGORY_DETAILS) as TagCategoryKey[]).forEach(k => { c[k] = new Set<string>(); }); games.forEach(g => { g.tags?.forEach(t => { if (t.categoryKey in c) c[t.categoryKey].add(t.name); }); }); const r: Record<string, string[]> = {}; (Object.keys(TAG_CATEGORY_DETAILS) as TagCategoryKey[]).forEach(k => { r[k] = Array.from(c[k]).sort((a,b)=>a.localeCompare(b,'fr')); }); return r; }, [games]);
  const activeTagFilterBadges = useMemo(() => Object.entries(selectedTagFilters).flatMap(([k,t]) => t.map(n => ({ categoryKey: k as TagCategoryKey, categoryName: getTranslatedTagCategory(k), tagName: n }))), [selectedTagFilters]);
    ...
    <>
      <Card>
        ...
        <CardContent className="space-y-2">
            <div className="border-2 border-dashed p-4 rounded-lg mb-6">
                <h4 className="font-semibold text-sm">Remplir la base de données</h4>
                <p className="text-xs text-muted-foreground mt-1">Utilisez cet outil une seule fois pour remplir la collection `games` avec vos données locales.</p>
                <Button variant="secondary" size="sm" className="mt-2" onClick={seedGamesDatabase} disabled={isSeeding}>{isSeeding ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Database className="mr-2 h-4 w-4"/>}{isSeeding ? 'Remplissage...' : 'Lancer le remplissage'}</Button>
            </div>
            ...
            <TableBody>{displayedGames.map((game) => (<TableRow key={game.id}><TableCell><div className="relative h-12 w-12 rounded overflow-hidden bg-muted">{game.imageUrl ? (<Image src={game.imageUrl} alt={`Image de ${game.name}`} fill sizes="50px" className="object-cover"/>) : (<div className="flex items-center justify-center h-full w-full"><Gamepad2 className="h-6 w-6 text-muted-foreground" /></div>)}</div></TableCell><TableCell className="font-medium">{game.name}</TableCell>{visibleColumns.publisher&&(<TableCell>{game.publisher||<span className="italic">N/A</span>}</TableCell>)}{visibleColumns.publicationYear&&(<TableCell>{game.publicationYear||<span className="italic">N/A</span>}</TableCell>)}{visibleColumns.tags&&(<TableCell>{game.tags&&game.tags.length>0?(<div className="flex flex-wrap gap-1">{game.tags.slice(0,3).map(t=>(<Badge key={`${t.categoryKey}-${t.name}`} variant="customColor" className={cn("font-normal text-xs", getTagCategoryColorClass(t.categoryKey))} title={`${getTranslatedTagCategory(t.categoryKey)}: ${t.name}`}>{t.name}</Badge>))} {game.tags.length>3&&(<Badge variant="outline" className="text-xs">+{game.tags.length-3}</Badge>)}</div>):(<span className="text-muted-foreground text-xs">Aucun</span>)}</TableCell>)}{visibleColumns.description&&(<TableCell>{game.description?(<Tooltip><TooltipTrigger asChild><p className="text-xs text-muted-foreground line-clamp-2 cursor-default">{game.description}</p></TooltipTrigger><TooltipContent><p className="text-xs">{game.description}</p></TooltipContent></Tooltip>):(<span className="text-muted-foreground text-xs italic">N/A</span>)}</TableCell>)}<TableCell className="text-right"><Button variant="ghost" size="icon" onClick={()=>handleOpenEditGameDialog(game)}><Edit className="h-4 w-4"/></Button><Button variant="ghost" size="icon" className="text-destructive hover:text-destructive/80" onClick={()=>handleDeleteGame(game.id, game.name)}><Trash2 className="h-4 w-4"/></Button></TableCell></TableRow>))}</TableBody></Table>
      ...
      <Dialog open={isGameFormDialogOpen} onOpenChange={setIsGameFormDialogOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader><DialogTitle>{isAddingGame ? "Ajouter un jeu" : `Modifier : ${currentGameToEdit?.name}`}</DialogTitle></DialogHeader>
