...
import { LibraryBig, ListFilter, Loader2, X, PlusCircle } from 'lucide-react';
...
export default function GamesPage() {
  ...
  const filteredGames = useMemo(() => {
    ...
    if (allSelected.length === 0) return allGames;
    return allGames.filter(g => { if (!g.tags?.length) return false; return (Object.keys(selectedFilters) as TagCategoryKey[]).every(k => { const s = selectedFilters[k]; if (s.length === 0) return true; return g.tags!.some(gt => gt.categoryKey === k && s.includes(gt.name)); }); });
  }, [selectedFilters, allGames]);
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      {filteredGames.length > 0 ? (
        ...
        <div className="text-center py-12"><p className="text-xl text-muted-foreground">{activeSelectedTags.length>0?`Aucun jeu ne correspond aux filtres.`:"Aucun jeu dans la bibliothèque."}</p>{activeSelectedTags.length>0&&(<Button variant="link" onClick={resetFilters} className="mt-2">Voir tous les jeux</Button>)}</div>
      )}
    </div>
  );
}