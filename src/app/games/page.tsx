...
export default function GamesPage() {
  ...
  const allCategorizedTags = useMemo(() => {
    ...
    allGames.forEach(game => { 
      game.tags?.forEach(tag => { 
        ...
        // On aide TypeScript en lui confirmant le type de la clé
        const categoryKey = tag.categoryKey as TagCategoryKey;
        if (categoryKey in categorized) { 
          categorized[categoryKey].add(tag.name); 
        } 
      }); 
    });
  ...
  const filteredGames = useMemo(() => {
    let games = allGames;
    if (searchQuery) { games = games.filter(game => game.name.toLowerCase().includes(searchQuery.toLowerCase())); }
    const allSelectedTagNames = Object.values(selectedFilters).flat();
    if (allSelectedTagNames.length > 0) { games = games.filter(game => { if (!game.tags?.length) return false; return (Object.keys(selectedFilters) as TagCategoryKey[]).every(k => { const s = selectedFilters[k]; if (s.length === 0) return true; return game.tags!.some(gt => gt.categoryKey === k && s.includes(gt.name)); }); }); }
    return games;
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      {filteredGames.length > 0 ? (
        ...
        <div className="text-center py-12 flex flex-col items-center"><Frown className="h-16 w-16 text-muted-foreground mb-4" /><p className="text-xl text-muted-foreground mb-2">{allGames.length > 0 ? `Aucun jeu ne correspond à vos critères.` : "Aucun jeu dans la bibliothèque."}</p>{(activeSelectedTags.length > 0 || searchQuery) && (<Button variant="link" onClick={resetFilters} className="mt-2">Voir tous les jeux</Button>)}</div>
      )}
    </div>
  );
}